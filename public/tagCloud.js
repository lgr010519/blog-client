window.tagcloud = (function(win, doc) {
    ...
    TagCloud.prototype = {
        ...
        _getItems: function () {
            var self = this,
                items = [],
                elememt = self.box.children,
                length = elememt.length,
                item;
            for (var i = 0; i < length; i++) {
                ...
                item.y =
                    ...
                    Math.sin(item.angle.phi);
                item.z =
                    self.radius * 1.5 * Math.cos(item.angle.phi);
                item.element.style.left =
                    item.x + (self.box.offsetWidth - item.offsetWidth) / 2 +
                    "px";
                item.element.style.top =
                    item.y + (self.box.offsetHeight - item.offsetHeight) / 2 +
                    "px";
                items.push(item);
            }
            return items;
